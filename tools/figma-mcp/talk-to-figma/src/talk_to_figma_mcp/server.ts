...
server.tool(
  "create_component",
  "Create a real native Figma component node",
  {
    x: z.number().describe("X position"),
    y: z.number().describe("Y position"),
    ...
    fillColor: z.object({ r: z.number().min(0).max(1), g: z.number().min(0).max(1), b: z.number().min(0).max(1), a: z.number().min(0).max(1).optional() }).optional(),
    strokeColor: z.object({ r: z.number().min(0).max(1), g: z.number().min(0).max(1), b: z.number().min(0).max(1), a: z.number().min(0).max(1).optional() }).optional(),
...
server.tool(
  ...
  "Combine existing component nodes into a Figma component set / variants",
  {
    componentIds: z.array(z.string()).min(2).describe("Component node IDs to combine"),
...
server.tool(
  "get_bounding_boxes",
  "Get absolute bounding boxes for nodes, useful for visual QA",
  { nodeIds: z.array(z.string()).describe("Node IDs to inspect") },
  async (params: any) => {
    try {
      const result = await sendCommandToFigma("get_bounding_boxes", params);
      return { content: [{ type: "text", text: JSON.stringify(result) }] };
    } catch (error) {
      return { content: [{ type: "text", text: `Error getting bounding boxes: ${error instanceof Error ? error.message : String(error)}` }] };
    }
  }
);

// Detect Overlaps Tool
server.tool(
  "detect_overlaps",
  "Detect rectangular overlaps among selected node bounding boxes",
  {
    nodeIds: z.array(z.string()).describe("Node IDs to compare"),
    tolerance: z.number().optional().describe("Minimum intersection width/height to count"),
    ignoreHidden: z.boolean().optional().describe("Ignore hidden nodes")
  },
  async (params: any) => {
    try {
      const result = await sendCommandToFigma("detect_overlaps", params);
      return { content: [{ type: "text", text: JSON.stringify(result) }] };
    } catch (error) {
      return { content: [{ type: "text", text: `Error detecting overlaps: ${error instanceof Error ? error.message : String(error)}` }] };
    }
  }
);
...
server.tool(
  ...
  {
    nodeId: z.string().describe("Node ID that supports prototype reactions"),
    reactions: z.array(z.any()).describe("Array of Figma Reaction objects")
  },
  async (params: any) => {
    try {
...
// Create Prototype Link Tool
server.tool(
  "create_prototype_link",
  "Create a simple clickable prototype link from a source node to a destination node",
  {
    sourceNodeId: z.string().describe("Source/hotspot node ID"),
    destinationId: z.string().optional().describe("Destination frame/node ID. Omit for BACK/CLOSE actions."),
    trigger: z.enum(["ON_CLICK", "ON_HOVER", "ON_PRESS", "ON_DRAG", "AFTER_TIMEOUT", "MOUSE_ENTER", "MOUSE_LEAVE"]).optional().describe("Prototype trigger"),
    navigation: z.enum(["NAVIGATE", "OVERLAY", "SWAP", "SCROLL_TO", "BACK", "CLOSE"]).optional().describe("Navigation/action type"),
    transition: z.enum(["INSTANT", "DISSOLVE", "SMART_ANIMATE", "MOVE_IN", "MOVE_OUT", "PUSH", "SLIDE_IN", "SLIDE_OUT"]).optional().describe("Transition type"),
    preserveExisting: z.boolean().optional().describe("Append instead of replacing existing reactions"),
    overlayRelativePosition: z.object({ x: z.number(), y: z.number() }).optional().describe("Overlay relative position for OVERLAY navigation")
  },
  async (params: any) => {
    try {
...
type CommandParams = {
  ...
  create_component: { x: number; y: number; width: number; height: number; name?: string; parentId?: string; fillColor?: { r: number; g: number; b: number; a?: number }; strokeColor?: { r: number; g: number; b: number; a?: number }; strokeWeight?: number };
  create_component_set: { componentIds: string[]; name?: string };
  get_bounding_boxes: { nodeIds: string[] };
  detect_overlaps: { nodeIds: string[]; tolerance?: number; ignoreHidden?: boolean };
  ...
  set_reaction: { nodeId: string; reactions: any[] };
  create_prototype_link: { sourceNodeId: string; destinationId?: string; trigger?: string; navigation?: string; transition?: string; preserveExisting?: boolean; overlayRelativePosition?: { x: number; y: number } };
  set_focus: {
    nodeId: string;
  };
  set_selections: {
