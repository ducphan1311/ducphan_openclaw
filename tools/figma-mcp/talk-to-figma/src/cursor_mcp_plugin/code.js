...
// OpenClaw local build: analytics disabled for privacy.
// Do not create/persist analytics client ids and do not post analytics messages.
...
async function handleCommand(command, params) {
  switch (command) {
    ...
    case "get_bounding_boxes":
      return await getBoundingBoxes(params);
    case "detect_overlaps":
      return await detectOverlaps(params);
    ...
    case "create_prototype_link":
      return await createPrototypeLink(params);
    case "set_focus":
...
async function createComponent(params) {
  const { x = 0, y = 0, width = 100, height = 100, name = "Component", parentId, fillColor, strokeColor, strokeWeight } = params || {};
  const component = figma.createComponent();
  component.x = x;
  component.y = y;
  component.resize(width, height);
  component.name = name;
  if (fillColor) component.fills = [{ type: "SOLID", color: { r: parseFloat(fillColor.r) || 0, g: parseFloat(fillColor.g) || 0, b: parseFloat(fillColor.b) || 0 }, opacity: parseFloat(fillColor.a) || 1 }];
  if (strokeColor) component.strokes = [{ type: "SOLID", color: { r: parseFloat(strokeColor.r) || 0, g: parseFloat(strokeColor.g) || 0, b: parseFloat(strokeColor.b) || 0 }, opacity: parseFloat(strokeColor.a) || 1 }];
  ...
  return { id: component.id, name: component.name, type: component.type, key: component.key, x: component.x, y: component.y, width: component.width, height: component.height, parentId: component.parent ? component.parent.id : undefined };
}

async function createComponentSet(params) {
  const { componentIds, name = "Component Set" } = params || {};
  if (!componentIds || !Array.isArray(componentIds) || componentIds.length < 2) throw new Error("componentIds must contain at least 2 component IDs");
  const components = [];
  for (const id of componentIds) {
    const node = await figma.getNodeByIdAsync(id);
    if (!node) throw new Error(`Component node not found: ${id}`);
    if (node.type !== "COMPONENT") throw new Error(`Node is not a COMPONENT: ${id}`);
    components.push(node);
  }
  const set = figma.combineAsVariants(components, components[0].parent || figma.currentPage);
  set.name = name;
  return { id: set.id, name: set.name, type: set.type, componentCount: set.children.length, children: set.children.map(c => ({ id: c.id, name: c.name, type: c.type })) };
}

function nodeBox(node) {
  const b = node.absoluteBoundingBox;
  return { id: node.id, name: node.name, type: node.type, x: b ? b.x : node.x, y: b ? b.y : node.y, width: b ? b.width : node.width, height: b ? b.height : node.height, visible: node.visible !== false };
}

async function getBoundingBoxes(params) {
  const { nodeIds } = params || {};
  if (!nodeIds || !Array.isArray(nodeIds)) throw new Error("nodeIds must be an array");
  const boxes = [];
  for (const id of nodeIds) {
    const node = await figma.getNodeByIdAsync(id);
    if (!node) { boxes.push({ id, error: "Node not found" }); continue; }
    boxes.push(nodeBox(node));
  }
  return { count: boxes.length, boxes };
}

async function detectOverlaps(params) {
  const { nodeIds, tolerance = 0, ignoreHidden = true } = params || {};
  if (!nodeIds || !Array.isArray(nodeIds)) throw new Error("nodeIds must be an array");
  const boxes = [];
  for (const id of nodeIds) {
    const node = await figma.getNodeByIdAsync(id);
    if (!node) continue;
    if (ignoreHidden && node.visible === false) continue;
  ...
  for (let i = 0; i < boxes.length; i++) for (let j = i + 1; j < boxes.length; j++) {
    ...
    if (w > tolerance && h > tolerance) overlaps.push({ a: { id: a.id, name: a.name, type: a.type }, b: { id: b.id, name: b.name, type: b.type }, intersection: { x: x1, y: y1, width: w, height: h, area: w * h } });
  }
  return { checked: boxes.length, overlapCount: overlaps.length, overlaps };
}
...
async function setReaction(params) {
  ...
  if (!Array.isArray(reactions)) throw new Error("reactions must be an array");
  const node = await figma.getNodeByIdAsync(nodeId);
  if (!node) throw new Error(`Node not found with ID: ${nodeId}`);
  if (!("reactions" in node)) throw new Error(`Node does not support prototype reactions: ${nodeId}`);
  node.reactions = reactions;
  return { id: node.id, name: node.name, type: node.type, reactions: node.reactions };
}

function buildPrototypeAction(params) {
  const { destinationId, navigation = "NAVIGATE", transition = "INSTANT", overlayRelativePosition } = params || {};
  if (navigation === "BACK") return { type: "BACK" };
  if (navigation === "CLOSE") return { type: "CLOSE" };
  if (!destinationId) throw new Error("destinationId is required for NODE navigation");
  const action = {
    type: "NODE",
    destinationId,
    navigation,
    preserveScrollPosition: false,
    transition: transition === "INSTANT" ? null : { type: transition }
  };
  if (overlayRelativePosition) action.overlayRelativePosition = overlayRelativePosition;
  return action;
}

async function createPrototypeLink(params) {
  const { sourceNodeId, destinationId, trigger = "ON_CLICK", navigation = "NAVIGATE", transition = "INSTANT", overlayRelativePosition, preserveExisting = false } = params || {};
  if (!sourceNodeId) throw new Error("Missing sourceNodeId parameter");
  const source = await figma.getNodeByIdAsync(sourceNodeId);
  if (!source) throw new Error(`Source node not found with ID: ${sourceNodeId}`);
