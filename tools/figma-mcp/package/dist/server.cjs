...
server.tool(
  ...
  "Create a real native Figma component node",
  {
    ...
    fillColor: import_zod.z.object({ r: import_zod.z.number().min(0).max(1), g: import_zod.z.number().min(0).max(1), b: import_zod.z.number().min(0).max(1), a: import_zod.z.number().min(0).max(1).optional() }).optional(),
    strokeColor: import_zod.z.object({ r: import_zod.z.number().min(0).max(1), g: import_zod.z.number().min(0).max(1), b: import_zod.z.number().min(0).max(1), a: import_zod.z.number().min(0).max(1).optional() }).optional(),
...
server.tool(
  ...
  {
    componentIds: import_zod.z.array(import_zod.z.string()).min(2).describe("Component node IDs to combine"),
    name: import_zod.z.string().optional().describe("Component set name")
  },
  async (params) => {
    try {
...
server.tool(
  ...
  {
    nodeIds: import_zod.z.array(import_zod.z.string()).describe("Node IDs to compare"),
    tolerance: import_zod.z.number().optional().describe("Minimum intersection width/height to count"),
    ignoreHidden: import_zod.z.boolean().optional().describe("Ignore hidden nodes")
  },
  async (params) => {
    try {
