...
for key in \
  ...
  FIGMA_API_TOKEN FIGMA_FILE_KEY FIGMA_TEAM_ID FIGMA_ORG_ID \
  GMAIL_ACCOUNT GMAIL_USER GMAIL_APP_PASSWORD \
  BROWSERACT_API_KEY APIFY_TOKEN \
  AMADEUS_API_KEY AMADEUS_API_SECRET AMADEUS_BASE_URL \
  AGENT_BRAIN_SUPERMEMORY_SYNC AGENT_BRAIN_PII_MODE AGENT_BRAIN_REMOTE_EMBEDDINGS; do
  sync_env_var "$key"
done
